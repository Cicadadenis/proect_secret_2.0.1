...
async def usse(call: CallbackQuery):
    ...
    if len(ss) <= 1:
        await call.answer("Список Пуст !", reply_markup=back_to_main_menu)
    sss = open('ussers.txt', 'r').readlines()
    if len(sss) > 50:
        await call.message.answer("Список слишком велик для отображения !", reply_markup=back_to_main_menu)
    else:
...
@dp.callback_query_handler(text_startswith="parserr")
async def start_spam(call: CallbackQuery, state: FSMContext):
    await call.answer("Скоро будет доступен !!")


@dp.callback_query_handler(text_startswith="invait")
async def start_am(call: CallbackQuery, state: FSMContext):
    await call.answer("Скоро будет доступен !!")
